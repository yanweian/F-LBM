...
inline LbmParameters recomputeParameters(LbmParameters lbmparameters)
{
    ...
    lbmparameters.niu = computeVec(lbmparameters.u0) * lbmparameters.Lxy.x / lbmparameters.Re;
    ...
    for (Index y = 0; y < lbmparameters.size.y; y++)
    {
        for (Index x = 0; x < lbmparameters.size.x; x++)
        {
            ...
            if (y == (lbmparameters.size.y - 1))
                u = computeVec(lbmparameters.u0);
            // init rho and u
            grid[idx].rho = lbmparameters.rho0;
            grid[idx].u = MakeVec(u, 0.f);
            // init f
            for (int i = 0; i < Q_VECTOR; i++)
            {
                grid[idx].f[i] = feq(grid[idx].rho, grid[idx].u, i);
            }
        }
    }
}
