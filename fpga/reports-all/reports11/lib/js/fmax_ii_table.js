    ...
    if (item.type == "text") {
      ...
      if ("links" in item) {
        for (var j = 0; j < item.links.length; j++) {
          var line_number = item.links[j].line;
          var file_name = item.links[j].filename;

          var createA = document.createElement("A");
          createA.appendChild(document.createTextNode(file_name + ":" + line_number));
      ...
      content = content + ": ";
      if (code_links.length > 0) {
        content = content + "(";
      }

      li.appendChild(document.createTextNode(content));

      for (var k = 0; k < code_links.length; k++) {
        if( k !== 0 ){
          li.appendChild(document.createTextNode(", "));
        }
        li.appendChild(code_links[k]);
      }
...
function renderLoopInfo(func_data, basicblock_info, loop_header_block, top_div) {
  for (var j = 0; j < func_data[loop_header_block].length; j++) {
    ...
    if (block_info.is_loop_header == "Yes" && block_info.name != loop_header_block) {
      ...
      var loop_location_str = "(" + file_name + ":" + line_number + ")";
...
function renderFunc(function_name, func_data, basicblock_info, tableBody) {
  ...
  var func_loc_str = "( " + file_name + ":" + line_number + " )";
  ...
  for (var i = 0; i < loops.length; i++) {
    ...
    if (loop_header_block in basicblock_info) {
      ...
      var file_name = loop_block.loop_location.details[0].links[0].filename;
      var line_number = loop_block.loop_location.details[0].links[0].line;
      var loop_location_str = "(" + file_name + ":" + line_number + ")";
      ...
      tr_loop.setAttribute("onclick", "syncEditorPaneToLine(" + line_number + "," + "\"" + file_name + "\")");
    }
